...
class UserRegistrationSerializer(serializers.ModelSerializer):
  ...
  class Meta:
    model = User
    fields=['email', 'name', 'password', 'password2', 'referred_by']
    extra_kwargs={
...
class SendPasswordResetEmailSerializer(serializers.Serializer):
  ...
  def validate(self, attrs):
    ...
    if User.objects.filter(email=email).exists():
      ...
      link = f'{settings.WEBHOST}/user/reset_password/'+uid+'/'+token
...
class UserReferralTokenSerializer(serializers.ModelSerializer):
    class Meta:
      model = UserReferral
      fields = ['token']